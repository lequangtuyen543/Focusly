    ...
    <>
      {isDesktop ? (
        ...
        <nav
          ...
          className="fixed bottom-[max(0.5rem,env(safe-area-inset-bottom))] left-1/2 z-40 flex w-[calc(100%-1rem)] max-w-[24rem] -translate-x-1/2 items-center justify-between rounded-full border border-outline-variant/20 bg-rich-black/90 px-2 py-2 shadow-nav-item backdrop-blur-xl"
        >
          {navItems.map(({ to, label }) => (
            <NavLink
              key={to}
              ...
              className={({ isActive }) =>
                `flex flex-1 min-h-[3.25rem] flex-col items-center justify-center rounded-full px-3 py-2 text-[11px] font-medium transition-all ${
                  isActive
